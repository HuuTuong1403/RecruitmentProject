...
const IssueAccountForm = (props) => {
  ...
  const {
    ...
    resolver: yupResolver(
      isAdmin ? schemaIssueAccountAdmin : schemaIssueAccountManager
    ),
  });
  ...
  const issueAccountManagerHandler = async (dataIssue) => {
    ...
    if (result.status === "success") {
      setLoading(false);
      notification(
        `${t("Successfully granting system management account")}`,
        "success"
      );
      reset();
    } else {
  ...
  const issueAccountAdminHandler = async (dataIssue) => {
    ...
    if (result.status === "success") {
      setLoading(false);
      notification(
        `${t("Successfully granting system administrator account")}`,
        "success"
      );
      reset();
    } else {
  ...
  return (
    <div className={classes.issueAccount}>
      <div className={classes.issueAccount__wrapped}>
        <div className={classes["issueAccount__wrapped--title"]}>
          {t(title)}
        </div>
        <form
          className={classes["issueAccount__wrapped--form"]}
          onSubmit={handleSubmit(
            isAdmin ? issueAccountAdminHandler : issueAccountManagerHandler
          )}
        >
          <div>
            <div>

...
import { statusEventOption, dateCreatedAtOptions } from 'common/constants/options'
...
export const SearchEventHeader = () => {
  ...
  const [selectProvince, setSelectProvince] = useState(query.get('address%city') ?? 'Tất cả')
  const [selectStatus, setSelectStatus] = useState(query.get('status') ?? 'Tất cả')
  ...
  const searchSubmitHandler = (e) => {
    ...
    if (
      ...
      const eventName =
        eventNameRef.current.value === '' ? '' : `eventName=${eventNameRef.current.value}&`
      const province = selectProvince === 'Tất cả' ? '' : `address%city=${selectProvince}&`
      const eventOrganizer =
        eventOrganizerRef.current.value === ''
          ? ''
          : `eventOrganizer=${eventOrganizerRef.current.value}&`
      const topic = topicRef.current.value === '' ? '' : `topic=${topicRef.current.value}&`
      const companyName =
        companyNameRef.current.value === '' ? '' : `companyName=${companyNameRef.current.value}&`

...
const ModalRegisteredInformation = ({
  createdAt,
  data,
  onCloseModal,
  showModal,
  ...
  const classStatus =
    status === "Not participate" ? classes.notParticipate : classes.participate;
    ...
    >
      <div className={classes.modalInfo}>
        ...
        <h3 className={classes.modalInfo__address}>
          {t("Event held address")}: {event.address.street},{" "}
          {event.address.ward}, {event.address.district}, {event.address.city}
        </h3>
          ...
          <div className={classes["modalInfo__content--field"]}>
            ...
            <div className={classes["modalInfo__content--field--interesting"]}>
              {interestingField.map((item, index) => (
                <Tooltip
                  key={index}
                  title={`${t("View jobs with skill")} ${item}`}
                >
                  <Link

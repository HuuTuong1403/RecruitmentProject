    ...
    jobsOfEmployer && (
      <div className={classes.recruitment}>
        ...
        {jobsOfEmployer.length === 0 ? (
          <NotFoundData title={t('Currently no job postings have been created')} />
        ) : (
          <Fragment>
            ...
            >
              <TabPane
                tab={`${t('unapproval')} (${mapArrayStatus(jobsOfEmployer, 'unapproval').length})`}
                key="unapproval"
              >
                <JobOfEmployerList
              ...
              <TabPane
                tab={`${t('denied')} (${mapArrayStatus(jobsOfEmployer, 'denied').length})`}
                key="denied"
              >
                <JobOfEmployerList
              ...
              <TabPane
                tab={`${t('approval')} (${mapArrayStatus(jobsOfEmployer, 'approval').length})`}
                key="approval"
              >
                <JobOfEmployerList
